...
__Note__ If we set $$\alpha=0$$, which corresponds to x being connected to the origin by an infinitely stiff spring then we get: $$\Delta x= -x$$ which is exactly the PBD update. i.e. just move the particle back to the point where it satisfies the constraint.


### XPBD (more involved)

The above should have established an intuition for what's going on. Let's dive into the paper a little deeper. We start with Newton's equations of motion:
...
## Building a differentiable PBD physics engine using JAX
